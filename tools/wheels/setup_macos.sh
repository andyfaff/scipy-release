...
if [[ "$BLAS_VARIANT" == "accelerate" ]]; then
    ...
    cibw_env="PKG_CONFIG_PATH='$PWD/.openblas' LIBRARY_PATH='$PWD/.openblas/lib' MACOSX_DEPLOYMENT_TARGET=$MACOSX_DEPLOYMENT_TARGET"
    # Set this on delocate itself: macOS system shells can strip inherited DYLD_*.
    echo "CIBW_REPAIR_WHEEL_COMMAND_MACOS=DYLD_LIBRARY_PATH='$PWD/.openblas/lib' delocate-wheel --require-archs {delocate_archs} -w {dest_dir} -v {wheel}" >> "$GITHUB_ENV"
fi
